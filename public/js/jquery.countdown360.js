...
(function ($, window, document, undefined) {
  let pluginName = 'countdown360',
    defaults = {
      ...
      label: ['min', 'mins', 'secs'], // the label to use or false if none
  ...
  Plugin.prototype = {
    getTimeRemaining() {
      const timeRemaining = this._secondsLeft(this.getElapsedTime());
    ...
    addSeconds(value) {
      const secondsElapsed = Math.round((new Date().getTime() - this.startedAt.getTime()) / 1000);
    ...
    _drawCountdownLabel(secondsElapsed) {
      ...
      if (minutesLeft == 0) {
        ...
        this.pen.fillText(minutesLeft + 1, x, y);
        this.pen.fillStyle = this.settings.fontColor;
    ...
    _drawCountdownShape(endAngle, drawStroke) {
      this.pen.fillStyle = this.settings.fillStyle;
      this.pen.beginPath();
